...
def item_common(letters, item)
  ...
  first.split('').each do |letter|
    letters << letter if second.split('').include? letter
  end

  letters.each do |letter|
    final << letter if third.split('').include? letter
  end
  final.uniq
end
...
def elf_groups(items)
  items.each_slice(3).to_a
end

def common_item(groups)
  letters = []
  groups.each do |group|
    letters << item_common(letters, group).join
  end
  letters
end
...
groups = elf_groups(input)

letters = common_item(groups)
